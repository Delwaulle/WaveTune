...
//piste1.addEventListener("click",changeTrack);
	...
	function changeTrack(event){
		...
		if(audio.paused){
			audio.play();
			document.getElementById("playPause").style.background="url(img/pause.png) no-repeat";	
		}
		selectedItem = this;
		selectedItem.style.color="blue";
	...
	function playPauseTrack(event){
		if(audio.paused){
			...
			audio.pause();
			document.getElementById("playPause").style.background="url(img/play.png) no-repeat";
		}
	}
